 ...
 */
static void push_extlinux(void)
{
        const char *mbr_src = TOP_DIR "/tests/data/gptmbr.bin";
        const char *mbr_dst = PLAYGROUND_ROOT "/usr/share/extlinux/gptmbr.bin";
        const char *mbr_dir = PLAYGROUND_ROOT "/usr/share/extlinux";
        fail_if(!nc_mkdir_p(mbr_dir, 00755), "Failed to create source extlinux tree");
