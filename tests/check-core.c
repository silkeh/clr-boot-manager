...
/* Needed due to extern */
#include "bootloaders/systemd-class.c"

#if defined(HAVE_SYSTEMD_BOOT)
        #include "bootloaders/systemd-boot.c"
#elif defined(HAVE_GUMMIBOOT)
        #include "bootloaders/gummiboot.c"
#else
        #include "bootloaders/goofiboot.c"
#endif
...
START_TEST(bootman_install_kernel_test)
{
        ...
        fail_if(!boot_manager_install_kernel(m, kernel),
                "Failed to install kernel");

        fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/loader/entries/Clear-linux-native-4.2.3-138.conf"),
                "Failed to find loader .conf entry");

        fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/org.clearlinux.native.4.2.3-138"),
...
START_TEST(bootman_set_default_kernel_test)
{
        ...
        fail_if(!boot_manager_set_default_kernel(m, kernel),
                "Failed to set default kernel");

        fail_if(!file_get_text(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/loader/loader.conf", &got),
...
START_TEST(bootman_remove_kernel_test)
{
        ...
        fail_if(!boot_manager_remove_kernel(m, kernel),
                "Failed to remove kernel");
...
START_TEST(bootman_purge_test)
{
        ...
        for (int i = 0; i < list->len; i++) {
                kernel = cbm_array_get(list, i);
                fail_if(!boot_manager_install_kernel(m, kernel),
                        "Failed to install known kernel");
        }
        ...
        for (int i = 0; i < list->len; i++) {
                kernel = cbm_array_get(list, i);
                fail_if(!boot_manager_remove_kernel(m, kernel),
                        "Failed to remove known kernel");
        }

        /* Second part of validation
        r = system("tree " TOP_BUILD_DIR "/tests/dummy_install");*/

        cbm_array_free(&list, (array_free_func)free_kernel);
...
START_TEST(bootman_install_bootloader_test)
{
        ...
        if (boot_manager_get_architecture_size(m) == 64) {
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/Boot/BOOTX64.EFI"),
                        "Main x64 bootloader missing");
#if defined(HAVE_SYSTEMD_BOOT)
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/systemd/systemd-bootx64.efi"),
                        "Systemd x64 bootloader missing");
#elif defined(HAVE_GUMMIBOOT)
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/gummiboot/gummibootx64.efi"),
                        "gummiboot x64 bootloader missing");
#else
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/goofiboot/goofibootx64.efi"),
                        "goofiboot x64 bootloader missing");
#endif
        } else {
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/Boot/BOOTIA32.EFI"),
                        "Main ia32 bootloader missing");
#if defined(HAVE_SYSTEMD_BOOT)
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/systemd/systemd-bootia32.efi"),
                        "systemd-boot ia32 bootloader missing");
#elif defined(HAVE_GUMMIBOOT)
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/gummiboot/gummibootia32.efi"),
                        "gummiboot ia32 bootloader missing");
#else
                fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/goofiboot/goofibootia32.efi"),
...
START_TEST(bootman_remove_bootloader_test)
{
        ...
        fail_if(!cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/Boot"),
...
#if defined(HAVE_SYSTEMD_BOOT)
                fail_if(cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/systemd"),
                        "Systemd x64 bootloader present");
#elif defined(HAVE_GUMMIBOOT)
                fail_if(cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/gummiboot"),
                        "gummiboot x64 bootloader present");
#else
                fail_if(cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/EFI/goofiboot"),
        ...
        fail_if(cbm_file_exists(TOP_BUILD_DIR "/tests/dummy_install/"BOOT_DIRECTORY"/loader/loader.conf"),
