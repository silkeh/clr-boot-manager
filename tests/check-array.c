...
START_TEST(cbm_array_add_check)
{
        ...
        fail_if(cbm_array_add(NULL, &data1), "Added data to NULL array");
        array = cbm_array_new();
        fail_if(!array, "Failed to allocate new array");
        fail_if(cbm_array_add(array, NULL), "Added NULL data to array");
        fail_if(!cbm_array_add(array, &data1), "Failed to add data1 to array");
        ...
        fail_if(cbm_array_add(array, &data1), "Able to add more than max number of elements");
        array->len = 1;
        /* Test resize */
        fail_if(!cbm_array_add(array, &data2), "Failed to add second element to array");
        ...
        *data3 = 3;
        array = cbm_array_new();
        fail_if(!array, "Failed to allocate new array");
        fail_if(!cbm_array_add(array, data3), "Failed to add pointer data");
...
START_TEST(cbm_array_get_check)
{
        ...
        array = cbm_array_new();
        fail_if(!array, "Failed to allocate new array");
        fail_if(cbm_array_get(NULL, 0), "Got data from NULL array");
        fail_if(cbm_array_get(array, 0), "Got data from empty array");
        fail_if(!cbm_array_add(array, &data1), "Failed to add data1 to array");
        fail_if(*((int *)cbm_array_get(array, 0)) != 1,
                "Failed to get correct value for element 0");
        fail_if(cbm_array_get(array, 1), "Got data past end of array");
        cbm_array_free(&array, NULL);
        fail_if(array, "Failed to set array to NULL");
}
END_TEST
...
START_TEST(cbm_array_check)
{
        ...
        fail_if(value == NULL, "Failed to get value from CBMArray");

        fail_if(strcmp(value, "test") != 0, "Failed to retrieve the stored value");

        cbm_array_free(&array, cbm_array_free_fun);
        fail_if(array != NULL, "Failed to free CBMArray");
}
END_TEST
